...
function Navigation() {
  ...
  /* method to extract tide times from response data*/
  ...
  // useRef is used to persist component states between page loads ... I believe
  ...
  useEffect(() => {
    ...
    axios.get(tideAPIcall)
      .then(response => setTideData(response.data)) // why am I not getting data here?, network tab in browser shows response data
      .catch(error => {
        console.log(error.response.data.error) // not hitting the catch because I'm getting response data
      })

      console.log(tideData);
    */
    // so axios is working, the call is working, it's only hitting the .then, I get a response in the network tab,
    // so the setter isn't working.... that's the only thing it could be,
    // so compare it to a setter that is working.

    /* GET DAILY CAUGHT FISH */
    axios.get('/tblZXiWg0iGnfIucV?fields%5B%5D=fishCaught&fields%5B%5D=date')
      .then((response) => {
        setFishCaughtData(response.data)
        console.log('done!',fishCaughtData)
      });
